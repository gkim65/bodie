...
double Kp = 20;//15.0;//40.0; //15.0;
double Kd = 1.7; //0.5;
double centerAngle = 0.5;
...
state estimateState(LSM6 imu) {
  ...
  // Serial.print("Omega_raw,");
  // Serial.println(omega);
  omegaHistory[0] = omega;

  // Basic-ass low pass filter on the omega
  double weights[4] = {1.0, 0.0,0.0,0};//0.05, 0.025, 0.025};
  ...
  double thetaFromGyro = omegaFiltered*dt + theta;
...
void loop()
{
  ...
  double inputVoltage1 = (Kp*(RobotState.pitch-centerAngle)) - Kd*RobotState.pitchRate;
  double inputVoltage2 = (Kp*(RobotState.pitch-centerAngle)) - Kd*RobotState.pitchRate;
  ...
  // delay(1000);
  // Write speed signal
  analogWrite(MOTOR1SPEEDPIN, abs(clippedMotorInput1));//*0.9);
  analogWrite(MOTOR2SPEEDPIN, abs(clippedMotorInput1));//*0.6);
